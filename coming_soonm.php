<?php include"header.php"; ?>

  <!--<main id="main">-->
                  
	<section id="middle-content" class="middle-content">
            <div class="container-fluid con-fluid1" data-aos="fade-up">
                <div class="row no-gutters card-shadow pr-3" data-aos="fade-up" data-aos-delay="100">
                    <div class="col-md-12">
                        <!--<center><button type="button" name="back" class="btn btn-info">Back To Home</button></center>-->
                       <img src="assets/Frame 21.jpeg" class="img-fluid" style=" width:100%;  height:80%;">
                    </div> 
                </div>
            </div> 
        </section>   
  
    
  <?php include"footer.php"; ?>