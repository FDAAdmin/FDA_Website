...
(function ($) {
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append('<div><img class="cover" src="assets/images/orgchart.minus.png"/></div>');
		}
		...
		if ($childNodesCount > 0) {		
			$nodeDiv.next().children('img.cover').click(function () {
				...
				if ($tr.hasClass('contracted')) {
					...
					$(this).attr('src', 'assets/images/orgchart.minus.png');
					...
					$(this).attr('src', 'assets/images/orgchart.plus.png');
		...
		if ($node.attr('class') != undefined) {
			...
			$.each(classList, function (index, item) {
				if (item == 'collapsed') {
					...
					$nodeRow.find('img.cover').attr('src', 'assets/images/orgchart.plus.png');
				} else {
